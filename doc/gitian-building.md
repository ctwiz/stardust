...
*Setup instructions for a Gitian build of Stardust Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Stardust
...
- [Building Stardust Core](#building-stardust-core)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-stardust-host/README.md).
...
Clone the git repositories for stardust and Gitian.
...
in the stardust repository under 'Fetch and build inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Stardust Core
----------------

To build Stardust Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the stardust repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/stardust/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/stardust/stardust
...
For example:
```bash
URL=https://github.com/laanwj/stardust.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit stardust=${COMMIT} --url stardust=${URL} ../stardust/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit stardust=${COMMIT} --url stardust=${URL} ../stardust/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit stardust=${COMMIT} --url stardust=${URL} ../stardust/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the stardust git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../stardust/contrib/gitian-descriptors/*|sort|uniq )
...
DSTPATH=/some/root/path/stardust
SIGPATH=/some/root/path/stardust-detached-sigs

./bin/gbuild --url stardust=${DSTPATH},signature=${SIGPATH} ../stardust/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/stardust-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/stardust-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/stardust-osx-build.assert
```
...
[stardust-core/gitian.sigs](https://github.com/stardust-core/gitian.sigs/) repository, or if that's not possible create a pull
