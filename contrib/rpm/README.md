...
The RPM spec file provided here is for Stardust-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://stardust.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Stardust git repository](https://github.com/stardust/stardust) but are not part of
...
Currently only `Source10` is used, for the example `stardust.conf` file.
...
in the stardust git repository. At present only an SVG version of the Stardust
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the stardust
...
needed when 0.12.1 is released, a proper fix is already in the Stardust git
master branch.
...
idea to build Stardust Core against a static version of that library compiled
...
The `stardust.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `stardust-server` package. At this point, most RPM based
...
The `stardust-server` package should have SELinux support. How to properly do
...
Stardust-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/stardust/). He has been
packaging Stardust for Fedora at least since 2012.
...
4. I renamed the `stardust` package that contains the Qt GUI to `stardust-core` as
that appears to be how the general population refers to it, in contrast to
`stardust-xt` or `stardust-classic`. I wanted to make sure the general population
