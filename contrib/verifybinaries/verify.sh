...
###   This script attempts to download the signature file SHA256SUMS.asc from stardust.org
...
WORKINGDIR="/tmp/stardust"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://stardust.org/bin/"
VERSIONPREFIX="stardust-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'stardust-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Stardust Core binary release signing key installed?"
   fi
